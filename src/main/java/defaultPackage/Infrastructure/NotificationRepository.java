...
public class NotificationRepository {
    ...
    public void Add(Notification notification) {
        ...
        param1.key = 1;
        param1.param = notification.id;
        var param2 = new DbParam();
        param2.key = 2;
        param2.param = notification.userId;
        var param3 = new DbParam();
        param3.key = 3;
        param3.param = notification.title;
        var param4 = new DbParam();
        param4.key = 4;
        param4.param = notification.notifyDateTime;
        var array = new ArrayList<DbParam>();
        array.add(param1);
        array.add(param2);
        array.add(param3);
        array.add(param4);
        // для каждой переменной '?' в запросе нужно создать собственный параметр, в
        // определенном порядке, начиная с 1.
        _dao.ExecuteUpdate("INSERT INTO notifications  (id, user_id, title, notify_date_time) VALUES (?, ?, ?, ?)",
                array);

    }

    public List<String> getList(long chatId) {
        try {
